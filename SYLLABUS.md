...
- [x] [Módulo 07: Sistemas, Inferencia Eficiente y Cuantización](#módulo-07-sistemas-inferencia-eficiente-y-cuantización) `[x]`
...
* **Notebooks del Módulo:**
  ...
  * `[x]` `03_rnns_lstms_and_bottlenecks.ipynb`: Modelado secuencial clásico, BPTT y el límite de paralelización en GPUs.
...
* **Notebooks del Módulo:**
  ...
  * `[x]` `02_diffusion_foundations_ddpm.ipynb`: Modelos de difusión probabilística (DDPM): forward process y reverse process intuitivos.
  * `[x]` `03_flow_matching_and_sampling.ipynb`: De difusión discreta a ODEs continuas y Flow Matching moderno.
...
* **Notebooks del Módulo:**
  ...
  * `[x]` `02_peft_and_lora_from_scratch.ipynb`: Adaptación de bajo rango (LoRA / QLoRA): formulación matemática e implementación manual.
  * `[x]` `03_preference_alignment_dpo.ipynb`: Direct Preference Optimization (DPO) vs pipelines clásicos de RLHF.
  * `[x]` `04_reasoning_and_test_time_compute.ipynb`: Dinámica de cadenas de pensamiento (CoT) y búsqueda en tiempo de inferencia.
...
* **Notebooks del Módulo:**
  ...
  * `[x]` `04_local_inference_and_onnx.ipynb`: Exportación a ONNX Runtime, GGUF/llama.cpp e inferencia local eficiente en CPU/GPU integrada.
